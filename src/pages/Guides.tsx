...
import { Clock, BarChart3, Monitor, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
      ...
      <div className="grid gap-4">
        {guides.map(guide => (
          <Link
            key={guide.id}
            to={`/guides/${guide.id}`}
            className="block p-6 rounded-lg border transition-colors duration-100"
            style={{ borderColor: 'var(--color-border)' }}
          >
            <div className="flex items-center gap-3 mb-3">
              <span className="text-xs font-mono px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--color-bg-tertiary)', color: 'var(--color-accent)' }}>
                {guide.protocol}
              </span>
...
export function GuideDetail() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <Link to="/guides" className="text-sm mb-6 inline-block" style={{ color: 'var(--color-text-muted)' }}>
        Все гайды
      </Link>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
              ...
              {guide.sections.map((section, i) => (
                <li key={section.id}>
                  <button
                    ...
                    className="text-sm text-left w-full py-1 px-2 rounded transition-colors duration-100"
                    style={{
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between mt-12 pt-8 border-t" style={{ borderColor: 'var(--color-border)' }}>
            {prevGuide ? (
              <Link to={`/guides/${prevGuide.id}`} className="flex items-center gap-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                <ChevronLeft size={16} /> {prevGuide.title}
              </Link>
            ) : <div />}
            {nextGuide ? (
              <Link to={`/guides/${nextGuide.id}`} className="flex items-center gap-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                {nextGuide.title} <ChevronRight size={16} />
              </Link>
            ) : <div />}
          </div>
        </div>
